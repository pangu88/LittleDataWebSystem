...
var rainbowloan_result = {};

rainbowloan_result.get = function (req, res) {
    // res.render("rainbowloan_result");
    var sid = req.query.sid;
    db_handle.query(`select count(*) from rainbow_record` 
    ...
    (r1)=>{
        ...
        if (cnt <= 0) {     // 说明没有找到 --> 还没有处理完
            res.render("rainbowloan_result", {
                ...
                tip2: `<a href="/rainbowloan_result?sid=${sid}">点击此处刷新</a>`
            });
        }
        else {              // 说明找到了 --> 已经处理完了
            var sql = `
            ...
            db_handle.query(sql, (r2)=>{
                ...
                fs.writeFile(fileOutputPath, buffer, (err3)=>{
                    ...
                    res.render("rainbowloan_result", {
                        ...
                        tip2: `<a href="/upload_files/${outFileName}" download="丰收彩虹带授信测算${sid}.xlsx">点击此处进行下载</a>`
                    });
                });
                setTimeout(()=>{
...
rainbowloan_result.post = function (req, res) {

}

module.exports = rainbowloan_result;